...
// defines how many frames may be worked on simultaneously at once
...
public:
  ...
  void createUniformBuffers() {
    ...
    // Vulkan. All Shader uniforms are passed via uniform buffer blocks
    VkMemoryRequirements memReqs;
  ...
  virtual void render() {
    ...
    uniform_data.projectionMatrix = camera.matrices.perspective;
    uniform_data.viewMatrix = camera.matrices.view;
    uniform_data.modelMatrix = glm::mat4(1.0f);
    ...
    memcpy(uniformBuffers[currentBuffer].mapped, &uniform_data,
           sizeof(UniformData));
    ...
    vkCmdBeginRenderPass(commandBuffers[currentBuffer], &renderPassBeginInfo,
    ...
    // Bind descriptor set for the currrent frame's uniform buffer, so the
    // shader uses the data from that buffer for this draw
    vkCmdBindDescriptorSets(commandBuffers[currentBuffer],
                            VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0,
                            1, &uniformBuffers[currentBuffer].descriptorSet, 0,
                            nullptr);
    ...
    vkCmdBindVertexBuffers(commandBuffers[currentBuffer], 0, 1,
                           &vertices.buffer, offsets);
    // Bind triangle index buffer
    vkCmdBindIndexBuffer(commandBuffers[currentBuffer], indices.buffer, 0,
    ...
    submitInfo.pCommandBuffers =
        &commandBuffers[currentBuffer]; // Command buffers(s) to execute in this
                                        // batch (submission)
